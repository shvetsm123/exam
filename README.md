...
# Финальные правки (ветка MAIN)

## 3. React Events and Timers
3.1. В EventForm валидация Yup вынесена в utils/validators
3.2. В EventForm убран шаблонный код согласно библиотеке Formik

## 8. Node.js Logger creation
8.1. errorLogger.js переписан на промисы

## 9. Full Stack Create moderation functionality
9.1. Все отправленные эл. письма хранятся в txt файле
9.2. Автоматическая смена moderStatus с pending на rejected при победе одного из офферов
